...
const LAST_UPDATED_DATE = DateTime.fromObject({
  year: 2023,
  month: 10,
  day: 31,
  hour: 12,
...
describe("RBCParcer", () => {
  ...
  describe("Main parsing method", () => {
    beforeEach(() => {
      axios.get
        .mockResolvedValueOnce({ data: testData[0] })
        .mockResolvedValueOnce({ data: testData[1] });
    });

    it("should parse news correctly for the set of data", async () => {
      const result =
        await parser.makeParceUntilPublicationDateTime(LAST_UPDATED_DATE);
  ...
  describe("_getDateTimeFromHtmlBlock", () => {
    it(`should extract datetime correctly`, () => {
      ...
      const spyConvertRBCDatetimeToISO = jest.spyOn(
        dateConverterServices,
        "convertRBCDatetimeToISO",
      );
    ...
    const wrongDateCases = ["32 окт, 15:06", "1", ""];
